...
void Layer_output(std::vector<float> output, std::array<int, 4> output_shape, std::string layername, std::ofstream& outFile) {
    outFile << "First Channel output values " << layername << " layer: \n";
    int output_height = output_shape[1];
    int output_width = output_shape[2];
    int num_channels = output_shape[3];

    outFile << "Output Height: " << output_height << std::endl;
    outFile << "Output Width: " << output_width << std::endl;
    outFile << "Number of Channels: " << num_channels << std::endl;

    for (int i = 0; i < output_height; ++i) {
        for (int j = 0; j < output_width; ++j) {
            int index = (i * output_width + j) * num_channels;
...
int main() {
    std::ifstream configFile("D:/MCW/Assignment-2/Assignment_2_test/configs/json/model_config_new.json");
    ...
    std::vector<float> input = readBinaryFile<float>(inputPath);
    ...
    for (const auto& layer : config["layers"]) {
        ...
        if (type == "Conv2D") {
            ...
            convolution(input, kernel, bias, output, input_shape, output_shape, kernel_size, strides, padding, layer["layer_name"]);
            input = output;

            Layer_output(output, output_shape, layer["layer_name"], outFile);
            ...
            maxpooling(input, output, input_shape, output_shape, pool_size, strides, padding, layer["layer_name"]);
            input = output;
            ...
            outFile << "First Channel output values " << layer["layer_name"] << " layer: \n";
            for (int i = 0; i < 10; ++i) {
                outFile << output[i] << " ";
            }
            outFile << std::endl;
            ...
            if (layer["attributes"]["activation"] == "relu") {
                ...
                // Ensure output_shape is set correctly for ReLU
...
// //     std::string inputPath = "D:/MCW/Assignment-2/resized_image_binary.bin";
// //     std::string kernelPath1 = "D:/MCW/Assignment-1/data/weights/convolution_kernel.bin";
// //     std::string biasPath1 = "D:/MCW/Assignment-1/data/weights/convolution_bias.bin";

// //     // File paths for the second convolution layer
// //     std::string kernelPath2 = "D:/MCW/Assignment-1/data/weights/convolution_1_kernel.bin";
// //     std::string biasPath2 = "D:/MCW/Assignment-1/data/weights/convolution_1_bias.bin";

// //     // File paths for the third convolution layer
// //     std::string kernelPath3 = "D:/MCW/Assignment-1/data/weights/convolution_2_kernel.bin";
// //     std::string biasPath3 = "D:/MCW/Assignment-1/data/weights/convolution_2_bias.bin";
...
//     std::vector<float>input,  kernel1, bias1, convOutput1, maxPoolOutput1;
//     readBinaryFile(inputPath, input);
//     //std::vector<float> input(32 * 32 * 3, 0.39f);
//     readBinaryFile(kernelPath1, kernel1);
//     readBinaryFile(biasPath1, bias1);

//     // Perform first convolution
//     convolve(input, kernel1, bias1, convOutput1, inputWidth, inputHeight, inputChannels, outputChannels1, kernelSize, stride, padding);
//     std::cout << "Output Shape after first Convolution layer: (" << outputChannels1 << ", " << inputHeight << ", " << inputWidth << ")" << std::endl;
...
//             std::cout << convOutput1[index] << " ";  // Print the value for the first channel
//         }
