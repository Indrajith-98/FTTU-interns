...
int MyMath::factorial(int n) {
  ...
  for (int i = 1; i <= n; ++i) {
    if (result > std::numeric_limits<int>::max() / i) {
      throw std::overflow_error("Factorial result is too large to fit in an int.");
    }
    result *= i;
  }
  return result;
...
int MyMath::fibonacci(int n) {
  ...
  for (int i = 2; i <= n; ++i) {
    if (b > std::numeric_limits<int>::max() - a) {
      throw std::overflow_error("Fibonacci result is too large to fit in an int.");
    }
    int temp = b;
    b = a + b;
    a = temp;
...
double MyMath::mean_squared_error(const std::vector<double>& y_true,
  ...
  if (y_true.empty() || y_pred.empty()) {
    throw std::invalid_argument("y_true and y_pred must not be empty.");
  }
