...
vector<pair<long, long>> sortWithFrequency(vector<long> v1){
    ...
    sort(v1.begin(), v1.end()); // the starting index position is changed from 1 to 0 ,i.e, changed v1.begin()+1 to v1.begin() 
    ...
    sort(v2.begin(), v2.end(),  comparecount); // the starting index position is changed from 1 to 0 ,i.e, changed v2.begin()+1 to v2.begin()
    return v2;
}
