...
vector<pair<long, long>> sortWithFrequency(vector<long> v1){
    ...
    sort(v1.begin(), v1.end()); // The First element of the vector v1 was ignored during sorting -> Fixed
    ...
    sort(v2.begin(), v2.end(),  comparecount); // The First element of the vector v2 was ignored during sorting -> Fixed
    return v2;
}
