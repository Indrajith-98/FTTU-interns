...
int MyMath::fibonacci(int n) {
  ...
  for (int i = 2; i <= n; ++i) {
    int temp = a + b;
    if (temp < a || temp < b) {  // Check for overflow
      throw std::overflow_error("Fibonacci result exceeds integer limits.");
    }
    a = b;
    b = temp;
  }
...
int MyMath::lcm(int a, int b) {
  ...
  if (a == 0 || b == 0) {
    ...
    if (a > std::numeric_limits<int>::max() / b) {
      throw std::overflow_error("LCM result exceeds integer limits.");
    }
    return (a * b) / gcd(a, b);
  }
}
...
std::vector<int> MyMath::prime_factors(int n) {
  ...
  for (int i = 2; i * i <= n; ++i) {
    while (n % i == 0) {
      if (factors.size() > std::numeric_limits<int>::max()) {
        throw std::overflow_error("Prime factors array exceeds size limits.");
      }
      factors.push_back(i);
      n /= i;
    }
...
double MyMath::calculate_pi(int num_iterations) {
  ...
  if (num_iterations > 100000) {
    throw std::overflow_error("Number of iterations exceeds the allowed limit.");
  }
