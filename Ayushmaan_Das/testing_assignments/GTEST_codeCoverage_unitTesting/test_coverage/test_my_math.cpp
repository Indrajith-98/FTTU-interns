...
TEST(FactorialTest, PositiveInputs)
{
    ...
    EXPECT_EQ(MyMath::factorial(0), 1); // Edge case: 0
    EXPECT_EQ(MyMath::factorial(1), 1); // Edge case: 1
}
...
{
    EXPECT_NO_THROW(MyMath::factorial(12));    // Ensure no overflow for large input
    EXPECT_EQ(MyMath::factorial(10), 3628800); // Check higher bound values
}

TEST(FibonacciTest, ValidInputs)
{
    EXPECT_EQ(MyMath::fibonacci(0), 0); // Edge case: 0
    EXPECT_EQ(MyMath::fibonacci(1), 1); // Edge case: 1
    EXPECT_EQ(MyMath::fibonacci(5), 5);
    EXPECT_EQ(MyMath::fibonacci(10), 55); // Larger value
}
...
TEST(GCDTest, ValidInputs)
{
    ...
    EXPECT_EQ(MyMath::gcd(101, 103), 1); // Prime numbers
    EXPECT_EQ(MyMath::gcd(0, 5), 5);     // Edge case: 0 and non-zero
    EXPECT_EQ(MyMath::gcd(5, 0), 5);     // Edge case: non-zero and 0
}
...
TEST(LCMTest, ValidInputs)
{
    ...
    EXPECT_EQ(MyMath::lcm(0, 5), 0); // Edge case: 0 and non-zero
    EXPECT_EQ(MyMath::lcm(4, 0), 0); // Edge case: non-zero and 0
    EXPECT_EQ(MyMath::lcm(1, 1), 1); // Minimal input
}

TEST(PrimeFactorsTest, BasicCases)
{
    EXPECT_EQ(MyMath::prime_factors(28), std::vector<int>({2, 2, 7}));
    EXPECT_EQ(MyMath::prime_factors(29), std::vector<int>({29})); // Prime number
    EXPECT_EQ(MyMath::prime_factors(1), std::vector<int>({}));    // Edge case: no prime factors
}
...
TEST(SigmoidTest, ValidInput)
{
    ...
    EXPECT_NEAR(MyMath::sigmoid(-1), 0.268, 0.01);        // Negative input
    EXPECT_NEAR(MyMath::sigmoid(10), 0.999, 0.001);       // Large positive input
    EXPECT_NEAR(MyMath::sigmoid(-10), 0.000045, 0.00001); // Large negative input
}

TEST(ReLUTest, ValidInput)
{
    EXPECT_EQ(MyMath::relu(1), 1);
    EXPECT_EQ(MyMath::relu(-1), 0);    // Negative input
    EXPECT_EQ(MyMath::relu(0), 0);     // Edge case: zero
    EXPECT_EQ(MyMath::relu(100), 100); // Large positive
    EXPECT_EQ(MyMath::relu(-100), 0);  // Large negative
}
...
{
    EXPECT_THROW(MyMath::calculate_pi(0), std::invalid_argument);  // Zero iterations
    EXPECT_THROW(MyMath::calculate_pi(-1), std::invalid_argument); // Negative iterations
}
