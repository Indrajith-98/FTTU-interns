...
void conv2d_1d(const vector<float> &input, const vector<float> &kernel,
  ...
  int output_height =
      (input_height + 2 * pad_height - kernel_height) / stride + 1;
          ...
          for (int kw = 0; kw < kernel_width; ++kw) {
            for (int ic = 0; ic < input_channels; ++ic) {
              ...
              if (ih >= 0 && ih < input_height && iw >= 0 && iw < input_width) {
                ...
                int kernel_idx =
                    ((kh * kernel_width + kw) * input_channels + ic) *
                        output_channels +
                    c;
  ...
  cout << "Conv2D Output Shape: [" << output_height << ", " << output_width
       << ", " << output_channels << "]" << endl;

  cout << "Execution Time: " << execution_time.count() << " seconds" << endl;

  ofstream outfile("../data/cpp_outputs/" + layername + ".txt");
  if (outfile.is_open()) {
    for (int h = 0; h < output_height; ++h) {
      for (int w = 0; w < output_width; ++w) {
    ...
    outfile.close();
    cout << "First channel of Conv2D output saved to "
            "conv2d_1st_channel_output.txt"
         << endl;
  } else {
