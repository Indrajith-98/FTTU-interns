...
void batch_normalization_1d(const std::vector<float> &input,
                            ...
                            float epsilon, size_t channels, size_t height,
                            size_t width, std::string layername) {
    ...
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        output[idx] = gamma[c] * (input[idx] - moving_mean[c]) /
                          std::sqrt(moving_variance[c] + epsilon) +
                      beta[c];
      }
    }
  }
  ...
  std::cout << "Execution time for batch_normalization_1d: "
...
void batch_normalization_1d1(const std::vector<float> &input,
                             std::vector<float> &output,
                             const std::vector<float> &gamma,
                             const std::vector<float> &beta,
                             const std::vector<float> &moving_mean,
  ...
  if (input.size() % channels != 0) {
    throw std::runtime_error(
        "Input size is not divisible by the number of channels.");
  }
  ...
  std::cout << "=====================================================\n";
  // Print execution time
  std::cout << "Execution time for batch_normalization_1d1: "
            << execution_time.count() << " seconds\n";
  ...
  for (size_t s = 0; s < spatial_size; ++s) {
    for (size_t c = 0; c < channels; ++c) {
      ...
      outfile
          << output[idx]
          << " "; // Write all channel outputs for the current spatial position
    }
    outfile << std::endl; // New line after all channels for the current spatial
                          // position
  }

  outfile.close();
  std::cout << "Output of all channels saved to data/cpp_outputs/" + layername +
                   ".txt"
            << std::endl;
  std::cout << "=====================================================\n";
}
