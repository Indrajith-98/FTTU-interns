...
// Function to read a binary file and push into a vector
template <typename T> vector<T> read_binary_file(const string &file_path) {
...
// Function to write a vector to the output file
template <typename T>
...
// Function to create a proper file path using filesystem
...
int main_new() {
  ...
  ifstream input_file("../../configs/config_file.json");
  ...
  for (const auto &layer : model["layers"]) {
    ...
    fs::path input_file_path =
        base_dir /
        layer["input_file_path"].get<string>(); // Correct path concatenation
    cout << "Debug: Input file path: " << normalize_path(input_file_path)
         << endl; // Normalize and print path
    output_file << "  Reading input from: " << normalize_path(input_file_path)
                << endl;
    if (fs::exists(input_file_path)) {
      try {
        auto input_data = read_binary_file<float>(input_file_path.string());
        output_file << "    Input Data (first 10 elements): ";
        write_vector(output_file,
                     vector<float>(input_data.begin(),
                                   input_data.begin() +
                                       min(input_data.size(), size_t(10))));
    ...
    for (const auto &weights_file_path : layer["weights_file_paths"]) {
      fs::path weight_file_path =
          base_dir /
          weights_file_path.get<string>(); // Correct path concatenation
      cout << "Debug: Weight file path: " << normalize_path(weight_file_path)
           << endl; // Normalize and print path
      output_file << "  Reading weights from: "
                  << normalize_path(weight_file_path) << endl;
      if (fs::exists(weight_file_path)) {
        try {
          auto weights_data =
              read_binary_file<float>(weight_file_path.string());
          output_file << "    Weights Data (first 10 elements): ";
          write_vector(output_file,
                       vector<float>(weights_data.begin(),
                                     weights_data.begin() +
                                         min(weights_data.size(), size_t(10))));
        } catch (const exception &e) {
    ...
    fs::path output_file_path =
        base_dir /
        layer["output_file_path"].get<string>(); // Correct path concatenation
    cout << "Debug: Output file path: " << normalize_path(output_file_path)
         << endl; // Normalize and print path
    output_file << "  Reading output from: " << normalize_path(output_file_path)
                << endl;
    if (fs::exists(output_file_path)) {
      try {
        ...
        write_vector(output_file,
                     vector<float>(output_data.begin(),
                                   output_data.begin() +
                                       min(output_data.size(), size_t(10))));
      } catch (const exception &e) {
