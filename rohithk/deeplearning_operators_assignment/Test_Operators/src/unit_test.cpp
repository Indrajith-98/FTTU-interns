...
std::ofstream result_file("../../report/unit_test_check.txt", std::ios::app);
...
void unit_test(const std::string &layer, const std::string &output_file,
               const std::string &expected_file, float epsilon) {
  ...
  if (output_data.size() != expected_data.size()) {
    std::cerr << "Test Failed: Size mismatch between output ("
              << output_data.size() << ") and expected ("
              << expected_data.size() << ") data." << std::endl;
    return;
  }
  ...
  if (test_passed) {
    result_file << layer << " Test Passed: All values match within tolerance ("
                << epsilon << ")." << std::endl;
  } else {
